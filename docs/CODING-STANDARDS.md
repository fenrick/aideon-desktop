...
- **Crate package names** (Cargo `[package].name`): kebab‑case (e.g., `aideon_praxis`); the crate library name defaults to snake_case; set `lib.name` only if needed for coherence.
